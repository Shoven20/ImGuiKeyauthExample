...
namespace KeyAuth {
	...
	public:
		...
		void init()
		{
			...
			auto data =
				("type=") + encryption::encode(("init")) +
				("&ver=") + encryption::encrypt(version, secret, iv) +
				("&enckey=") + encryption::encrypt(enckey, secret, iv) +
...
//				Sleep(3500);
		...
		void regstr(std::string username, std::string password, std::string key) {
			...
			auto iv = encryption::sha256(encryption::iv_key());
			auto data =
				("type=") + encryption::encode("register") +
				("&username=") + encryption::encrypt(username, enckey, iv) +
				("&pass=") + encryption::encrypt(password, enckey, iv) +
		...
		void upgrade(std::string username, std::string key) {
			...
			auto iv = encryption::sha256(encryption::iv_key());
			auto data =
				("type=") + encryption::encode("upgrade") +
				("&username=") + encryption::encrypt(username, enckey, iv) +
				("&key=") + encryption::encrypt(key, enckey, iv) +
				("&sessionid=") + encryption::encode(sessionid) +
				("&name=") + encryption::encode(name) +
		...
		std::string var(std::string varid) {
			...
			auto iv = encryption::sha256(encryption::iv_key());
			auto data =
				("type=") + encryption::encode("var") +
				("&varid=") + encryption::encrypt(varid, enckey, iv) +
				("&sessionid=") + encryption::encode(sessionid) +
				("&name=") + encryption::encode(name) +
				("&ownerid=") + encryption::encode(ownerid) +
				("&init_iv=") + iv;
		...
		void log(std::string message) {
			...
			auto data =
				("type=") + encryption::encode(("log")) +
				("&pcuser=") + encryption::encrypt(UsernamePC, enckey, iv) +
				("&message=") + encryption::encrypt(message, enckey, iv) +
				("&sessionid=") + encryption::encode(sessionid) +
				("&name=") + encryption::encode(name) +
				("&ownerid=") + encryption::encode(ownerid) +
				("&init_iv=") + iv;
		...
		std::vector<unsigned char> download(std::string fileid) {
			...
			auto data =
				("type=") + encryption::encode("file") +
				("&fileid=") + encryption::encrypt(fileid, enckey, iv) +
				("&sessionid=") + encryption::encode(sessionid) +
				("&name=") + encryption::encode(name) +
				("&ownerid=") + encryption::encode(ownerid) +
				("&init_iv=") + iv;
		...
		void webhook(std::string id, std::string params) {
			...
			auto data =
				("type=") + encryption::encode("webhook") +
				("&webid=") + encryption::encrypt(id, enckey, iv) +
				("&params=") + encryption::encrypt(params, enckey, iv) +
				("&sessionid=") + encryption::encode(sessionid) +
				("&name=") + encryption::encode(name) +
				("&ownerid=") + encryption::encode(ownerid) +
				("&init_iv=") + iv;
	...
	private:
		...
		static std::string req(std::string data) {
			...
			curl_easy_setopt(curl, CURLOPT_URL, ("https://keyauth.win/api/1.0/"));
